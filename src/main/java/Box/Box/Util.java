	...
	private ArrayList<ContainerIndexes> checkIfContainsKnot(List<Token> tokens2, int sta, int end) {
		...
		while (count < end) {
			...
			for (int i = startIndex; i < end; i++) {
				if (tokens2.get(i).type == TokenType.OPENPAREN && !tokens2.get(i).isFlow) {
					if (start) {
						start = false;
						startIndex = i;
					}
					...
					countOpenClose++;
				} else if (tokens2.get(i).type == TokenType.OPENBRACE && !tokens2.get(i).isFlow) {
					if (start) {
						start = false;
						startIndex = i;
					}
					...
					countOpenClose++;
				} else if (tokens2.get(i).type == TokenType.CLOSEDBRACE && !tokens2.get(i).isFlow) {
					endIndex = i;
					if (stack.size() > 0) {
				...
				} else if (tokens2.get(i).type == TokenType.CLOSEDPAREN && !tokens2.get(i).isFlow) {
					endIndex = i;
					if (stack.size() > 0) {
