...
public class Grouper {
	...
	private void addBangTagsToIdents(ArrayList<Token> tokens2) {
		...
		for (int i = 0; i < tokens2.size(); i++) {
			if (i >= 0 && i + 2 < tokens2.size() - 1) {
				if (tokens2.get(i).type == TokenType.CLOSEDBRACE && !tokens2.get(i).isFlow && tokens2.get(i + 1).type == TokenType.IDENTIFIER
					...
					tokens2.remove(i + 2);
						...
						&& tokens2.get(i + 2).type == TokenType.OPENBRACE && !tokens2.get(i + 2).isFlow) {
	...
	private void addColonTagsToIdents(ArrayList<Token> tokens2) {
		for (int i = 0; i < tokens2.size(); i++) {
			// Opening: COLON IDENTIFIER (SINGLEAND IDENTIFIER)* (HASH IDENTIFIER)? OPENSQUARE
			if (i + 2 < tokens2.size()
					&& tokens2.get(i).type == TokenType.COLON
					&& tokens2.get(i + 1).type == TokenType.IDENTIFIER) {
				int j = i + 2;
				StringBuilder sb = new StringBuilder(":");
				sb.append(tokens2.get(i + 1).lexeme);
				while (j + 1 < tokens2.size() && tokens2.get(j).type == TokenType.SINGLEAND
						&& tokens2.get(j + 1).type == TokenType.IDENTIFIER) {
					sb.append("&").append(tokens2.get(j + 1).lexeme);
					j += 2;
				}
				if (j + 1 < tokens2.size() && tokens2.get(j).type == TokenType.HASH
						&& tokens2.get(j + 1).type == TokenType.IDENTIFIER) {
					sb.append("#").append(tokens2.get(j + 1).lexeme);
					j += 2;
				}
				if (j < tokens2.size() && tokens2.get(j).type == TokenType.OPENSQUARE) {
					tokens2.get(i + 1).lexeme = sb.toString();
					tokens2.get(i + 1).literal = sb.toString();
					tokens2.get(i + 1).identifierToken = tokens2.get(i);
			...
			// Closing: CLOSEDSQUARE IDENTIFIER (HASH IDENTIFIER)? (SINGLEAND IDENTIFIER)* COLON
			if (i + 2 < tokens2.size()
					&& tokens2.get(i).type == TokenType.CLOSEDSQUARE
					&& tokens2.get(i + 1).type == TokenType.IDENTIFIER) {
				int j = i + 2;
				StringBuilder sb = new StringBuilder(tokens2.get(i + 1).lexeme);
				if (j + 1 < tokens2.size() && tokens2.get(j).type == TokenType.HASH
						&& tokens2.get(j + 1).type == TokenType.IDENTIFIER) {
					sb.append("#").append(tokens2.get(j + 1).lexeme);
					j += 2;
				...
				if (j < tokens2.size() && tokens2.get(j).type == TokenType.COLON) {
					sb.append(":");
					tokens2.get(i + 1).lexeme = sb.toString();
					tokens2.get(i + 1).literal = sb.toString();
					tokens2.get(i + 1).reifitnediToken = tokens2.get(j);
					for (int k = j; k > i + 1; k--)
						tokens2.remove(k);
					continue;
				}
	...
	private void addHashTagsToIdents(ArrayList<Token> tokens2) {
		...
		for (int i = 0; i < tokens2.size(); i++) {
			// Opening chain: HASH IDENTIFIER (SINGLEAND IDENTIFIER)* (GREATERTHEN IDENTIFIER)?
			if (i + 1 < tokens2.size() && tokens2.get(i).type == TokenType.HASH
					&& tokens2.get(i + 1).type == TokenType.IDENTIFIER) {
				int j = i + 2;
				...
				if (j + 1 < tokens2.size() && tokens2.get(j).type == TokenType.GREATERTHEN
						&& tokens2.get(j + 1).type == TokenType.IDENTIFIER) {
					sb.append(">").append(tokens2.get(j + 1).lexeme);
					j += 2;
					hasChain = true;
			...
			// Closing chain: IDENTIFIER (LESSTHEN IDENTIFIER)? (SINGLEAND IDENTIFIER)* HASH
			if (i + 2 < tokens2.size() && tokens2.get(i).type == TokenType.IDENTIFIER) {
				int j = i + 1;
				boolean hasChain = false;
				StringBuilder sb = new StringBuilder(tokens2.get(i).lexeme);
				if (j + 1 < tokens2.size() && tokens2.get(j).type == TokenType.LESSTHEN
						&& tokens2.get(j + 1).type == TokenType.IDENTIFIER) {
					sb.append("<").append(tokens2.get(j + 1).lexeme);
					j += 2;
					hasChain = true;
				...
				if (hasChain && j < tokens2.size() && tokens2.get(j).type == TokenType.HASH) {
					sb.append("#");
					tokens2.get(i).lexeme = sb.toString();
					tokens2.get(i).literal = sb.toString();
					for (int k = j; k > i; k--)
						tokens2.remove(k);
					continue;
				}
	...
	private void renameBetweenStartAndEnd(int start, int end, String lexeme, ArrayList<Token> tok) {
		...
		for (int i = start + 1; i < end; i++) {
			if (tok.get(i).type == TokenType.OPENPAREN && !tok.get(i).isFlow) {
			...
			} else if (tok.get(i).type == TokenType.OPENBRACE && !tok.get(i).isFlow) {
				if (tok.get(i).identifierToken == null) {
					String lexeme2 = lexeme + "_" + count+"{";
					Token ident = new Token(TokenType.IDENTIFIER, lexeme2, null, null, null, -1, -1, -1, -1);
					ident.column = tok.get(i).column;
			...
			} else if (tok.get(i).type == TokenType.CLOSEDPAREN && !tok.get(i).isFlow) {
			...
			} else if (tok.get(i).type == TokenType.CLOSEDBRACE && !tok.get(i).isFlow) {
				if (tok.get(i).reifitnediToken == null) {
					String reverse = reverse(lexeme);
					String lexeme2 = "}"+count + "_" + reverse;
					Token ident = new Token(TokenType.IDENTIFIER, lexeme2, null, null, null, -1, -1, -1, -1);
	...
	private ArrayList<ContainerIndexes> checkIfContainsBox(ArrayList<Token> tok, int sta, int end) {
		...
		while (count < end) {
			...
			for (int i = startIndex; i < end; i++) {
				if (tok.get(i).type == TokenType.OPENSQUARE && !tok.get(i).isFlow) {
					if (start) {
						start = false;
						startIndex = i;
					}
					...
					countOpenClose++;
				} else if (tok.get(i).type == TokenType.CLOSEDSQUARE && !tok.get(i).isFlow) {
					endIndex = i;
					if (stack.size() > 0) {
						stack.pop();
	...
	private int countUpClosedBoxes(ArrayList<Token> tokens2) {
		...
		for (Token token : tokens2) {
			if ((token.type == TokenType.CLOSEDSQUARE || token.type == TokenType.TEMPLID) && !token.isFlow)
				count++;
		}
		return count;
	}
	...
	private int countUpClosedPockets(ArrayList<Token> tokens2) {
		int count = 0;
		for (Token token : tokens2) {
			if ((token.type == TokenType.CLOSEDPAREN || token.type == TokenType.TEMPLID) && !token.isFlow)
				count++;
		}
		return count;
	}
	...
	private int countUpClosedCups(ArrayList<Token> tokens2) {
		int count = 0;
		for (Token token : tokens2) {
			if ((token.type == TokenType.CLOSEDBRACE || token.type == TokenType.TEMPLID) && !token.isFlow)
				count++;
		}
		return count;
	}
	...
	private boolean anyUndamedOpenOrClosed() {
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (isOpen(token)) {
				if (token.identifierToken == null && token.reifitnediToken == null && !token.isFlow)
					return true;
			}
			if (isClosed(token)) {
				if (token.identifierToken == null && token.reifitnediToken == null && !token.isFlow)
	...
	private int countUpOpenBoxes(ArrayList<Token> tokens2) {
		int count = 0;
		for (Token token : tokens2) {
			if (token.type == TokenType.OPENSQUARE && !token.isFlow)
				count++;
		}
		return count;
	}
	...
	private int countUpOpenCups(ArrayList<Token> tokens2) {
		int count = 0;
		for (Token token : tokens2) {
			if (token.type == TokenType.OPENBRACE && !token.isFlow)
				count++;
		}
		return count;
	}
	...
	private int countUpOpenPockets(ArrayList<Token> tokens2) {
		int count = 0;
		for (Token token : tokens2) {
			if (token.type == TokenType.OPENPAREN && !token.isFlow)
				count++;
		}
		return count;
	}
