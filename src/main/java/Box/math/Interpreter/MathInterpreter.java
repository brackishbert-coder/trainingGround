...
public class MathInterpreter implements Term.Visitor<Object> {
	...
	public BigDecimal C(BigDecimal x) {
		BigDecimal total = BigDecimal.ZERO;
		int sign = x.compareTo(BigDecimal.ZERO);
		boolean isNeg = false;
		if (sign == -1) { isNeg = true; x = x.negate(); }

		int cmp3 = x.compareTo(BigDecimal.valueOf(3.0));
		if (cmp3 == -1 || cmp3 == 0) {
			// Simpson's rule: ∫₀ˣ cos(πt²/2) dt
			int N = 1000;
			BigDecimal h = x.divide(BigDecimal.valueOf(N), new MathContext(999));
			for (int i = 0; i <= N; i++) {
				double t = h.multiply(BigDecimal.valueOf(i)).doubleValue();
				BigDecimal ft = BigDecimal.valueOf(Math.cos(Math.PI * t * t / 2));
				if (i == 0 || i == N)       total = total.add(ft);
				else if (i % 2 == 1)         total = total.add(ft.multiply(BigDecimal.valueOf(4)));
				else                         total = total.add(ft.multiply(BigDecimal.valueOf(2)));
			}
			total = total.multiply(h).divide(BigDecimal.valueOf(3), new MathContext(999));
		} else {
			// Asymptotic expansion for large x: C(x) ≈ 1/2 + sin/(πx) - cos/(π²x³) - 3sin/(π³x⁵) + ...
			double xd = x.doubleValue();
			BigDecimal cos = BigDecimal.valueOf(Math.cos(Math.PI * xd * xd / 2));
			BigDecimal sin = BigDecimal.valueOf(Math.sin(Math.PI * xd * xd / 2));
			BigDecimal pi  = BigDecimal.valueOf(Math.PI);
			BigDecimal term1 = BigDecimal.valueOf(0.5);
			BigDecimal term2 = sin.divide(pi.multiply(x), new MathContext(999));
			BigDecimal term3 = cos.divide(pi.pow(2).multiply(x.pow(3)), new MathContext(999)).negate();
			BigDecimal term4 = BigDecimal.valueOf(3).multiply(sin.divide(pi.pow(3).multiply(x.pow(5)), new MathContext(999))).negate();
			BigDecimal term5 = BigDecimal.valueOf(15).multiply(cos.divide(pi.pow(4).multiply(x.pow(7)), new MathContext(999)));
			BigDecimal term6 = BigDecimal.valueOf(105).multiply(sin.divide(pi.pow(5).multiply(x.pow(9)), new MathContext(999)));
			BigDecimal term7 = BigDecimal.valueOf(945).multiply(cos.divide(pi.pow(6).multiply(x.pow(11)), new MathContext(999))).negate();
			total = term1.add(term2.add(term3.add(term4.add(term5.add(term6.add(term7))))));
		}
		return isNeg ? total.negate() : total;
	}
