...
public class Environment {
	...
	private final java.util.Set<String> seedGlobals = new java.util.HashSet<>();
	private final Map<String, String> seedGlobalPaths = new HashMap<>();

	public void tagAsSeedGlobal(String name, String filePath) {
		seedGlobals.add(name);
		if (filePath != null) seedGlobalPaths.put(name, filePath);
	}

	public boolean isSeedGlobal(String name) {
		if (seedGlobals.contains(name)) return true;
		return enclosing != null && enclosing.isSeedGlobal(name);
	}

	public java.util.Set<String> getSeedGlobalNames() {
		return java.util.Collections.unmodifiableSet(seedGlobals);
	}

	public String getSeedGlobalPath(String name) {
		String path = seedGlobalPaths.get(name);
		if (path != null) return path;
		return enclosing != null ? enclosing.getSeedGlobalPath(name) : null;
	}
...
//				|| typesOfObject.getRunTimeTypeForObject() == RunTimeTypes.Any)
	...
	public java.util.Collection<Object> allLocalValues() {
